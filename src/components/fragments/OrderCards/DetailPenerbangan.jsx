...
const DetailPenerbangan = () => {
  const { flightData, loading, error } = useBooking();
  ...
  const totalPrice =
    (flightData.price.adults?.price || 0) * (flightData.price.adults?.count || 0) + (flightData.price.tax || 0);
  ...
  return (
    <div className="flex flex-col w-full lg:w-96">
      <div className="bg-white p-5 rounded-md shadow-md">
        <div className="flex justify-between mb-4">
          <span className="text-gray-600">Booking Code:</span>
          <span className="text-purple-600 font-semibold">{flightData.bookingCode || "N/A"}</span>
        </div>

        <div className="mb-6">
          <div className="flex justify-between text-sm">
            <span className="text-gray-600">{flightData.departure.time}</span>
            <span className="text-purple-600">Keberangkatan</span>
          </div>
          <div className="text-sm">{flightData.departure.date}</div>
          <div className="font-medium">{flightData.departure.location}</div>
        </div>

        <div className="mb-6">
          <div className="font-medium">
            {flightData.flight.airline} - {flightData.flight.class}
          </div>
          <div className="text-gray-600">{flightData.flight.code}</div>
          <div className="mt-2">
            <div className="flex items-center gap-2 text-sm text-gray-600">
              <Plane size={16} className="text-yellow-500" />
              <span>Informasi:</span>
            ...
            <ul className="text-sm text-gray-600 ml-6">
              <li>Baggage {flightData.flight.info.baggage}</li>
              <li>Cabin baggage {flightData.flight.info.cabinBaggage}</li>
              <li>{flightData.flight.info.entertainment}</li>
            </ul>
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="flex justify-between text-sm">
            <span className="text-gray-600">{flightData.arrival.time}</span>
            <span className="text-purple-600">Kedatangan</span>
          </div>
          <div className="text-sm">{flightData.arrival.date}</div>
          <div className="font-medium">{flightData.arrival.location}</div>
        </div>
        ...
        <div className="border-t pt-4">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span>{flightData.price.adults.count} Adults</span>
              <span>IDR {flightData.price.adults.price.toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span>{flightData.price.baby.count} Baby</span>
              <span>IDR {flightData.price.baby.price}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span>Tax</span>
              <span>IDR {flightData.price.tax.toLocaleString()}</span>
            </div>
            <div className="flex justify-between font-semibold text-purple-600 pt-2 border-t">
              <span>Total</span>
