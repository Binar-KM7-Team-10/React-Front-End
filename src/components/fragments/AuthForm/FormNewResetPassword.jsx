...
const ResetPasswordForm = () => {
  ...
  const onSubmit = async (data) => {
    if (token) {
      const result = await handleResetPassword(token, data.newPassword, data.confirmNewPassword);
      if (result) {
        setTimeout(() => {
          navigate('/login') 
        }, 3000);
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="relative">
            <div className="mb-4">
              ...
              <div className="relative">
                ...
                >
                  {showPassword ? <FiEyeOff style={{ color: "#8A8A8A" }} /> : <FiEye style={{ color: "#8A8A8A" }} />}
                </button>
              </div>
              {errors.newPassword && (
            ...
            <div className="mb-4">
              ...
              {errors.confirmNewPassword && (
                <p className="text-red-500 text-sm mt-2">{errors.confirmNewPassword.message}</p>
              )}
            </div>
